...
public:
	...
	void operator =(BoxContainer<T>&& source); // Move copy assignment operator
	void invalidate() {
		m_items = nullptr;
		m_size = 0;
		m_capacity = 0;
...
{
	std::cout << "BoxContainer move constructor called. Moving " 
						<< source.m_size << " items...\n";
...
void BoxContainer<T>::add(const T& item){
	std::cout << "Copy add method called...\n";
...
void BoxContainer<T>::add(T&& item){
	std::cout << "Move add method called...\n";
	if (m_size == m_capacity)
		//expand(m_size+5); // Let's expand in increments of 5 to optimize on the calls to expand
		expand(m_size + EXPAND_STEPS);
	// m_items[m_size] = item;
