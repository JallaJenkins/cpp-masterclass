...
int main(){
  ...
  std::cout << "2d array sum: " << result << '\n';

  result = sum_3d(weights_3d, std::size(weights_3d));
  std::cout << "3d array sum: " << result << '\n';
...
double sum(const double array[][3], size_t size){
  ...
  {
    for(size_t j{}; j < 3; ++j)  // Loop through elements in a single row
    {
      sum += array[i][j];  // Array access notation
      // sum += *( *(array + i) + j);  // Pointer arithmatic notation
    }
  }
  return sum;
}
...
double sum_3d(const double array[][3][2], size_t size){
  ...
  for(size_t i{}; i < size; ++i)  // Loop through rows
  {
    ...
    {
      for(size_t k{}; k < 2; ++k) {
        // sum += array[i][j][k];  // Array access notation
        sum += *(*( *(array + i) + j) + k);  // Pointer arithmatic notation
      } 
    }
  }
  return sum;
